...
#Calculates how many places (horizontal+vertical) the tile should be moved so that it reaches its correct position
...
def successor(node,level):
    ...
    if 0<=shift_posn[0]<=2 and 0<=shift_posn[1]<=2:
        ...
        heuristics = h2(down_shift_node[1], goal)+level+1 # f=g+h
    ...
    if 0<=shift_posn[0]<=2 and 0<=shift_posn[1]<=2:
        ...
        heuristics = h2(up_shift_node[1], goal)+level+1
    ...
    if 0<=shift_posn[0]<=2 and 0<=shift_posn[1]<=2:
        ...
        heuristics = h2(right_shift_node[1], goal)+level+1
    ...
    if 0<=shift_posn[0]<=2 and 0<=shift_posn[1]<=2:
        ...
        heuristics = h2(left_shift_node[1], goal)+level+1
